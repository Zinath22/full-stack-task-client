...
const AllUsersCard = () => {
  const { data, isLoading, isError, refetch } = useQuery({
    ...
    queryFn: async () => {
      try {
        const res = await axios.get('https://full-stack-task-server.vercel.app/users');
        return res.data;
      } catch (error) {
  ...
  const handleUserDelete = (user) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`https://full-stack-task-server.vercel.app/users/${user._id}`).then((res) => {
          ...
          <tbody className="bg-blue-400">
            {(filteredData.length > 0 ? filteredData : data).map((user) => (
              <tr key={user._id}>
                ...
                <td>
                  <button className="text-lg text-red-600 font-bold" onClick={() => handleUserDelete(user)}>X</button>
                </td>
              </tr>
            ))}
          </tbody>
